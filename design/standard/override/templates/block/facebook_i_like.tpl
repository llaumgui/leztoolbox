...
<div class="block-type-facebook_ilike" id="facebook_ilike_{$block.id}">
    ...
    $(document).ready(function(){ldelim} $object_code='<object data="http://www.facebook.com/plugins/like.php?href={$block.custom_attributes.url_to_like}&amp;layout={$layout_style}&amp;show_faces={$block.custom_attributes.show_faces}&amp;width=100%&amp;action={$verb_to_display}&amp;font=&amp;colorscheme={$color_scheme}&amp;height={$block.custom_attributes.height}" style="border:none;overflow:hidden;width:100%;height:{$block.custom_attributes.height}px;"></object>';$('#replace_facebook_ilike_{$block.id}').replaceWith($object_code);{rdelim});
	//]]></script>
</div>
